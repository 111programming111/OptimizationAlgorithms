...
class GeneticAlgorithm():
    ...
    def __init__(self, IterationsNumber=10, populationLength=6, CrossoverRatio=0.7, MutateRatio=0.001, minValue=1, maxValue=31):
        ...
        self.BinaryNumberLength = len(
            self.ConvertIntegerNumberToBinary(maxValue))
    ...
    def CumulativeRatio(self, fitness_Ratio: list):
        ...
        for i in range(len(fitness_Ratio)):
            if (i == 0):
                cumulative_Ratio.append(
                    [fitness_Ratio[i][0], round(fitness_Ratio[i][1], 2)])
            else:
                cumulative_Ratio.append([fitness_Ratio[i][0], round(
                    fitness_Ratio[i][1] + cumulative_Ratio[i-1][1], 2)])
    ...
    def PopulasyonCaprazla(self, newpopulasyon: list):
        for i in range(0, self.populationLength, 2):
            newItmes = self.KromozomCaprazlama(
                newpopulasyon[i], newpopulasyon[i+1], self.CrossoverRatio)
    ...
    def CheckToPoulasyon(self, mutasyonPopulasyon: list, fitness: list):
        ...
        for i in range(len(mutasyonPopulasyon)):
            fitnessTest.append(
                [mutasyonPopulasyon[i], self.Binary2Int(mutasyonPopulasyon[i])])
