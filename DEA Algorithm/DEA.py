...
class DEA():
     ...
     def MakeWeightingForVector(self):
          result = [a - b for a, b in zip(self.populasyon[self.vectorIndexList[0]], self.populasyon[self.vectorIndexList[1]])]
     ...
     def MakeMutasyon(self,caprazlamaResult:list):
          result = [a + b for a, b in zip(self.populasyon[self.mutasyonIndex[0]], caprazlamaResult)]
          return result
     ...
     def Run(self):
          ...
          for i in range(self.IterationsNumber):
              for j in range(self.populasyonLength):
                    ...
                    theSelectedNewKromozom = self.SelectGenFromKromozom(self.targetKromozom,mutsyonResultKromozom)
                    theSelectedNewKromozom = self.GetTheBetterResult(theSelectedNewKromozom,self.targetKromozom)
