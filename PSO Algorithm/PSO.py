...
class PSO():
    ...
    def Run(self):
        ...
        self.positions = np.random.uniform(-10,
                                           10, (self.parca_sayisi, self.d)).tolist()
        ...
        # Uygunluk hesaplama
        self.fitness = np.array([self.fun(x1, x2)
                                for x1, x2 in self.positions]).tolist()
        ...
        self.ShowChart(self.global_best_X1,
                       self.global_best_X2, self.global_fit)
        ...
        for i in range(self.parca_sayisi):
            for j in range(self.d):
                velocities[i][j] = (self.w * velocities[i][j]
                                    ...
                                    (self.best_positions[i]
                                     [j] - self.positions[i][j])
                                    + self.c2 * np.random.rand() *
                                    (self.global_best_posation[j] -
                                     self.positions[i][j])
                                    )
